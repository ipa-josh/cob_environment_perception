...
class Hypothesis(EKF):
	def __init__(self, trans, pose):
		...
		self.subs = {x: 0, y: 0, v:0, a:0, 
					 time:dt, w:self.robot.wheelbase, theta:0}
		self.x_x, self.x_y, = x, y 
		self.v, self.a, self.theta = v, a, theta

	def predict(self, u, dt):
		self.x = self.move(self.x, u, dt)
		...
		M = array([[self.robot.std_vel*u[0]**2, 0], 
				   [0, self.robot.std_steer**2]])
	...
	def move(self, x, u, dt):
		...
		if abs(steering_angle) > 0.001: # is robot turning?
			beta = (dist / self.robot.wheelbase) * tan(steering_angle)
			r = self.robot.wheelbase / tan(steering_angle) # radius
