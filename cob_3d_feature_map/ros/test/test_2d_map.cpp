...
TEST(ft_map, test_2d)
{
  ...
  typedef SI::FT FT;
  typedef SI::CR CR;
  typedef SI::INST INST;
  typedef SI::CL CL;
  typedef SI::COR COR;
  typedef SI::COR_SET COR_SET;
  ...
  for(size_t c=0; c<clusters.size(); c++) {
    for(size_t i=0; i<clusters[c]->getInstances().size(); i++)
    {
      ...
      for(size_t j=0; j<clusters[c]->getInstances()[i]->getFeatures().size(); j++) {
        ...
        tree.find_within_range_ex( *clusters[c]->getInstances()[i]->getFeatures()[j],0,back_insert_iterator2<vector<FT> >(howClose),back_insert_iterator2<vector<vector<float> > >(descr));
  ...
  for(size_t c=0; c<5; c++) {
    ...
    for(size_t i=0; i<sets[c].getCors().size(); i++) {
      ...
      float x1 = sets[c].getCors()[i].getOrigin()->getRepresentation().getMean()(0)*20 + 344/100.f+200;
      float y1 = sets[c].getCors()[i].getOrigin()->getRepresentation().getMean()(1)*20 + 344/100.f;
      float x2 = sets[c].getCors()[i].getBestMatch()->getRepresentation().getMean()(0)*20 + 344/100.f;
      float y2 = sets[c].getCors()[i].getBestMatch()->getRepresentation().getMean()(1)*20 + 344/100.f;
